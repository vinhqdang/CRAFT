...
\subsection{Mixture (Hedged) Betting}
\label{sec:mixture-method}

Sections~\ref{sec:ccp-negative} and~\ref{sec:snow-corruption-attempt} establish, across three real experiments, that neither the covariate-blind nor the CCP-informed bettor dominates consistently: on real Snowy Scenes data the CCP-informed bettor is strictly worse (Table~\ref{tab:snowy-results}), while on real CADC data it ties or marginally beats the covariate-blind bettor (Section~\ref{sec:cadc-results}). There is no way, for a new deployment, to know in advance which regime applies without already having a labeled onset stream to test against -- exactly the real-world situation a monitor is supposed to help with, not presuppose. Rather than committing to one bettor a priori, we run multiple candidate bettors' wealth processes in parallel over the same stream and combine them into a single e-process via a fixed convex combination.

\paragraph{Validity.} At any fixed timestep $t$, each component bettor's wealth $K_t^{(i)}$ is an e-value under $H_0$: nonnegative, with $\mathbb{E}_{H_0}[K_t^{(i)}] \le 1$ (Section~\ref{sec:eprocess}'s Step 1, applied recursively from $t=0$, exactly as already used to justify treating $K_t^{(i)}$ as an e-value for the spatial e-BH procedure in Section~\ref{sec:spatial}). Vovk and Wang~\cite{VovkWang2021Evalues} show that any convex combination of valid e-values is itself a valid e-value: for weights $w_1, \ldots, w_n \ge 0$ summing to 1, chosen without looking at the future (in our instantiation, fixed in advance, not adapted online),
\begin{equation}
K_t^{\mathrm{mix}} = \sum_{i=1}^{n} w_i \, K_t^{(i)}
\label{eq:mixture}
\end{equation}
satisfies $\mathbb{E}_{H_0}[K_t^{\mathrm{mix}}] \le \sum_i w_i \cdot 1 = 1$ at every fixed $t$, by linearity of expectation. Applying Ville's inequality to $K_t^{\mathrm{mix}}$ exactly as Section~\ref{sec:eprocess} already does for a single bettor's $K_t$ gives the same anytime-valid guarantee for the mixture: alarming the first time $K_t^{\mathrm{mix}} \ge 1/\delta$ controls the false-alarm rate at level $\delta$, with no additional correction for having hedged across $n$ component strategies rather than committing to one. This is what makes hedging provably safe rather than merely empirically convenient -- the combination rule is the same arithmetic averaging Section~\ref{sec:spatial}'s e-BH multiplicity control already relies on for a different purpose (combining many spatial cells' e-values rather than few candidate strategies' e-values), so no new validity machinery is introduced beyond what this paper already uses.

\paragraph{Instantiation.} We combine the two bettors already defined (Sections~\ref{sec:blind},~\ref{sec:informed}) at fixed equal weight, $w_{\mathrm{blind}} = w_{\mathrm{ccp}} = 0.5$ -- the simplest, most defensible instantiation, requiring no tuning and trivially valid by Equation~\ref{eq:mixture}. Both component bettors observe the same $m(t)$ and CCP disagreement $\overline{(1-S_t)}$ at every frame (the model is run once per frame; both bettors' own wealth processes are stepped from that single frame's evidence, not independently resampled data), and the mixture combines their wealth \emph{values} at each timestep, not their betting rates $\lambda_t$ -- the combination Equation~\ref{eq:mixture} is proved for. An adaptive, regret-weighted refinement (weighting each component by its own relative performance so far, rather than a fixed $50/50$ split) is a natural next step once this fixed instantiation's real behavior is understood, and we return to it in Section~\ref{sec:conclusion} rather than introduce it here without first establishing whether the simplest version helps at all.
