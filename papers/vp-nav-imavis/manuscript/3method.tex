...
We observe a stream of driving frames indexed by $t = 1, 2, \ldots$, one per LiDAR sweep. At every $t$, the base 3D detector $f_\theta$ described above produces per-object box predictions from the frame's fused camera/LiDAR BEV features, via a center-point / heatmap detection head in the style of~\cite{Zhou2019ObjectsAsPoints,Yin2021CenterPoint}: a per-cell object-presence heatmap $H$ locates candidate object centers and a co-located regression map $B$ predicts each object's box parameters at that cell; the monitor itself is detector-agnostic and does not depend on $f_\theta$'s specific architecture beyond this head structure, which is what lets Section~\ref{sec:null-diagnostic}'s null-detector control zero $B$ while leaving $H$ intact. We wrap those predictions in split conformal prediction, calibrated once on a held-out set of frames drawn from the nominal (least-degraded) regime available in the data. For object $i$ in frame $t$, the nonconformity score is the $\ell_1$ box-regression residual restricted to grid cells a ground-truth object actually occupies,
\[
...
Alongside the two contributions in Sections~\ref{sec:spatial} and~\ref{sec:experiments}, we also investigated conditioning the betting rate on an external signal available at the same timestep as $m(t)$ but computed from independent evidence: the disagreement output of the monitored detector's own CCP, $1-S$ (Equation~\ref{eq:ccp}). We describe the mechanism because it motivates this paper's monitoring design, and note here that it is specified and implemented but not evaluated in this paper (Section~\ref{sec:not-reported}). Our covariate-informed bettor wraps either covariate-blind base bettor and scales its stake by the frame's mean disagreement,
\[
\lambda_t = \operatorname{clip}\!\Big(\lambda_t^{\text{base}} \cdot \big(1 + \kappa \cdot \overline{(1 - S_t)}\big), \, 0, \, \lambda_{\max}\Big),
\]
where $\kappa \ge 0$ gates the covariate's influence ($\kappa = 0$ recovers the base bettor exactly) and $\overline{(1-S_t)}$ is the frame- (or cell-)averaged disagreement. The intended mechanism is that a scene beginning to degrade shows rising cross-modal disagreement before enough individual object detections have actually missed coverage for $m(t)$ to move -- disagreement as a leading indicator, miscoverage as the lagging one the covariate-blind rules are stuck reacting to. Whether that intended mechanism holds on real data is not settled in this paper; the rule is specified and implemented, and no result for it is reported here (Section~\ref{sec:not-reported}).

Figure~\ref{fig:qualitative-multi} shows why we did not lean on this
covariate, on real frames rather than in the abstract. It shows six real
Snowy Scenes val-split frames run through the trained checkpoint --- three
\texttt{accumulated} (nominal) and three \texttt{falling} (degraded),
spread across each category's frame pool rather than consecutive samples
--- with the camera image, the LiDAR bird's-eye-view occupancy channel, and
the CCP disagreement map $1-S$ for each. The per-frame mean disagreement is
$0.611$, $0.583$, $0.576$ for the three \texttt{accumulated} frames and
$0.564$, $0.559$, $0.573$ for the three \texttt{falling} frames --- category
means of $0.590$ (nominal) versus $0.565$ (degraded), consistently the
opposite of the ordering the covariate is meant to provide, and consistent
across all six frames rather than an artifact of one pair. Our best
explanation is that the CCP is trained to stay invariant under an
adversarial-robustness objective (Section~\ref{sec:background-detector}),
which is a qualitatively different feature-space disturbance from real
snowfall, and invariance learned against one does not obviously transfer to
sensitivity toward the other. This is why Section~\ref{sec:not-reported}
declines to report a result for the covariate-informed bettor rather than
reporting one anyway.

\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{figures/qualitative_multi.pdf}
\caption{Six real Snowy Scenes val-split frames run through the trained
checkpoint: three \texttt{accumulated} (nominal) and three \texttt{falling}
(degraded), each showing the camera image, LiDAR BEV occupancy channel, and
the CCP disagreement map $1-S$. Mean disagreement is consistently
\emph{higher} for the visually milder \texttt{accumulated} frames (category
mean $0.590$) than the visually more severe \texttt{falling} frames
(category mean $0.565$) --- the wrong-direction pattern this section
diagnoses, visible here across six independent real frames rather than
only in aggregate.}
...
\begin{tabular}{@{}p{1.8cm}p{4.4cm}p{4.6cm}@{}}
\toprule
...
CCP-informed & multiplicative covariate gain $\kappa$ on either base rule & inherits base rule's tradeoff; adds covariate sensitivity (specified and implemented; not evaluated here) \\
\bottomrule
\end{tabular}
\end{threeparttable}
\end{table}
...
\subsection{Session-Conditional (Mondrian) Calibration}
\label{sec:method-mondrian}

Split conformal calibration pools nonconformity scores and produces one
quantile, valid on average over the calibration distribution. Real driving
data arrives in sessions that differ persistently in weather, lighting,
traffic and route, and Section~\ref{sec:exchangeability} measures the
resulting per-session violation directly. Mondrian conformal
prediction~\cite{VovkMondrian2003,VovkGammermanShafer2005} conditions
calibration on a taxonomy, giving validity within each category; here the
category is the driving session.

Mondrian requires calibration data in every category it is queried about,
which forces a within-session split: each session calibrates on its own
leading frames and is monitored on the remainder. This is the
deployment-faithful arrangement --- a vehicle calibrates on the early
known-nominal portion of the drive it is currently on, rather than assuming
a quantile estimated on previous drives transfers --- and degraded frames
are scored against the quantile of the session the stream began in, which
is what a deployment does when conditions change mid-drive. Because
calibration and monitored frames then sit adjacent in time within a
correlated stream, we insert a temporal gap between them and sweep its
size; an effect inflated by adjacency shrinks as the gap grows.

\subsection{A Phantom-Detection Nonconformity Score}
\label{sec:method-phantom}

The score of Section~\ref{sec:setup} is a box-regression residual
restricted to cells a ground-truth object occupies, and is therefore
structurally blind to spurious detections in empty space. Snow's dominant
effect on LiDAR is precisely spurious near-range returns, so we add a
second score family: the predicted peak class activation at cells
containing no ground-truth object, calibrated by the same split-conformal
procedure at level $\alpha$. The frame-level rate is the convex combination
\begin{equation}
m(t) = (1 - w)\, m_{\mathrm{loc}}(t) + w\, m_{\mathrm{phantom}}(t),
\label{eq:phantom-mix}
\end{equation}
with $w \in [0,1]$. Each component is calibrated at level $\alpha$ and so
has expectation at most $\alpha$ under the nominal distribution; a convex
combination of two such quantities inherits the bound, which is exactly the
$H_0$ condition the wealth process of Section~\ref{sec:eprocess} requires.
No new theory is needed: the conformal guarantee is agnostic to the choice
of score function. Setting $w = 0$ recovers the original score exactly.

\subsection{Block Aggregation}
\label{sec:method-blocking}

Averaging $m$ over blocks of $k$ consecutive frames before the betting step
trades temporal resolution for reduced variance in the quantity the bettor
consumes. Under independent frames the reduction would be $\sqrt{k}$;
driving frames are strongly correlated, so the realised gain is smaller;
we do not report a measurement of it here. We keep one
trajectory entry per frame regardless of $k$, so detection delays remain in
frame units and are comparable across block sizes and against the
unblocked baseline.
