...
- [ ] **Fix the CCP-collapse regression this uncovered — now the critical path.** Running `scripts/run_real_operating_curve.py` against the trained checkpoint on held-out `val` frames shows the CCP-informed and covariate-blind bettors producing byte-identical operating curves. Confirmed cause: the trained CCP score `S` collapsed to ≈0.9993 uniformly (std ≈0.0001–0.0002) across both nominal and degraded regimes, because `compute_ccp_loss` is only ever called on the clean branch with the dataset's all-ones matched mask — the adversarial branches' own CCP scores (`s_adv_cam`, `s_adv_lid`) are computed but never given a mismatched-mask CCP loss, so training has zero incentive for `S` to ever move off 1.0. This directly invalidates claim 1 (CCP-informed betting beats covariate-blind) as currently implemented — see README's "First trained run and a real negative result" for full detail. Needs a fix to `craf_x/training/adversarial.py` (feed the adversarial-branch CCP scores into `compute_ccp_loss` with `m=0`) plus a retrain and a re-run of the real operating-curve script before any of claim 1's numbers can be trusted.
