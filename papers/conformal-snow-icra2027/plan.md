...
- [x] Full ~40.5GB download completed: 32 drives (18 bare across `2018_03_06`/`2018_03_07`, 14 covered from `2019_02_27`), 39GB, 2,942 annotated frames (1,650 bare, 1,292 covered).
- [x] Train `CRAFX_Net` on CADC from scratch (5 epochs, same protocol as Snowy Scenes: `128×128` BEV, batch size 4, Adam, lr `1e-4`, local GPU). Loss converged `4.68 → ~1.3` (detection component alone to `~0.002`), without the CCP loss collapsing to a constant — checkpoints in `checkpoints/cadc/`.
...
- [x] Ran it against the real trained checkpoint. **First run** (2 calibration drives, both from `2018_03_06`) gave false-alarm rate `1.00` at every δ for both bettors — a real calibration failure, not a bettor result. Diagnosed directly (per-drive coverage across the 16 held-out nominal drives, computed from real nonconformity scores): coverage ranged `0.47`–`0.91` and correlated cleanly with collection date (`2018_03_06` drives all below the `α=0.2` target of `≥0.80`; `2018_03_07` drives all above it) — a *representativeness* problem (the 2 calibration drives happened to share a date), not a size problem. **Fixed** by stratifying the calibration/nominal split per date (`_split_calibration_and_nominal`: half of each date's drives to calibration, half to the nominal stream — 9 calibration drives / 850 frames, 9 nominal drives / 800 frames, both spanning both dates). Rerunning gives false-alarm rate `0.00` at every δ for both bettors: covariate-blind detects the onset at delay 6/8/9 frames (δ=0.30/0.10/0.05); CCP-informed at delay 6/7/8 frames — tied at δ=0.30, one frame faster at δ=0.10 and δ=0.05. Real results in `real_operating_curve_cadc_run.json`.
- [x] Reported honestly: this is a **modest, CADC-specific reversal** of the Snowy Scenes negative finding — not evidence CCP-informed betting "works" in general (Snowy Scenes failed it across three separate training attempts; CADC shows only a one-frame-at-best improvement on a five-replicate pilot with no significance test). Both the calibration-heterogeneity finding and the CCP reversal are folded into `papers/vp-nav-imavis/` (the not-yet-submitted IMAVIS/VP-NAV journal extension — see its own `plan.md` and `manuscript/4exp.tex` §4.3.1/§4.4, `manuscript/5discussion.tex` §5.4), since `papers/conformal-snow-icra2027/manuscript/` itself remains locked/untouched.
