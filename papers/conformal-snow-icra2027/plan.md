...
- [ ] Full ~40.5GB download (in progress in the background as of this writing).
- [ ] Train `CRAFX_Net` on CADC from scratch (own config/num_classes, smoke-tested first).
- [ ] Build the onset-stream evaluation: if the real `bare`-vs-`covered` road-condition split gives a genuine detectable contrast, prefer it over a constructed splice; run the same calibration → operating-curve pipeline (`conformal_monitor/evaluate.py`), covariate-blind vs. CCP-informed.
- [ ] Report the real numbers honestly, whatever they are — including if this reproduces the CCP-informed-betting negative finding, or if it doesn't.
