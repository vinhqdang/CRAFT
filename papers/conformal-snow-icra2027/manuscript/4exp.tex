...
Table~\ref{tab:operating-curve} reports the operating curve against the corrected checkpoint (Section~\ref{sec:training}), evaluated on held-out \texttt{val}-split frames the detector never saw during training.

% RESULTS-PLACEHOLDER: fill in with the corrected real_operating_curve_run.json
% once checkpoints/snowy_scenes_fixed/checkpoint_final.pth finishes training
% and scripts/run_real_operating_curve.py has been re-run against it. Do not
% report the stale collapsed-CCP numbers here.

\begin{table}[t]
\centering
\caption{Operating curve on the real Snowy Scenes onset stream (accumulated $\to$ falling), 5 onset + 5 stationary-accumulated replicates per condition, corrected checkpoint. FAR = false-alarm rate; delay is in frames relative to the true onset at $t=8$; censored = replicates that never alarmed within the 20-frame window.}
...
\midrule
Covariate-blind (aGRAPA) & 0.30 & -- & -- & --/5 \\
Covariate-blind (aGRAPA) & 0.10 & -- & -- & --/5 \\
Covariate-blind (aGRAPA) & 0.05 & -- & -- & --/5 \\
CCP-informed & 0.30 & -- & -- & --/5 \\
CCP-informed & 0.10 & -- & -- & --/5 \\
CCP-informed & 0.05 & -- & -- & --/5 \\
\bottomrule
...
\begin{tablenotes}
\small
\item[] Placeholder: table populated after the corrected retrain (Section~\ref{sec:training}) completes and \texttt{scripts/run\_real\_operating\_curve.py} is re-run against \texttt{checkpoints/snowy\_scenes\_fixed/checkpoint\_final.pth}.
\end{tablenotes}
\end{threeparttable}
\end{table}

% DISCUSSION-PLACEHOLDER: once the table above is filled in, discuss here
% whether the CCP-informed bettor now shows a real advantage over the
% covariate-blind baseline at matched delta, or whether the fix did not
% restore one -- report the honest outcome either way and connect it back
% to the collapse story in Section~\ref{sec:training}.
