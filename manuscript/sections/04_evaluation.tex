...
\begin{table}[h]
\centering
\caption{Ablation Study of CRAF-X Components on Robust mAP (nuScenes, Attack A3)}
\begin{tabular}{ccc|c}
\toprule
\textbf{CCP (Consistency Probe)} & \textbf{GAFM (Gated Fusion)} & \textbf{ACT (Adversarial Training)} & \textbf{Robust mAP (A3)} \\
\midrule
- & - & - & 22.1 \\
\checkmark & - & - & 31.5 \\
\checkmark & \checkmark & - & 49.8 \\
\checkmark & \checkmark & \checkmark & \textbf{65.8} \\
\bottomrule
\end{tabular}
\label{tab:ablation_components}
\end{table}

The Baseline without any defense components reduces to a standard BEVFusion architecture (22.1 mAP). Adding the Consistency Probe (CCP) as a passive anomaly filter provides minor improvements (+9.4 mAP). However, the critical leap occurs when integrating the Gated Adaptive Fusion Module (GAFM), which actively quarantines adversarial features (+18.3 mAP). Finally, the Adversarial Consistency Training (ACT) synchronizes the detector and the fusion gate under attack profiles, pushing the robust performance to 65.8 mAP. 
